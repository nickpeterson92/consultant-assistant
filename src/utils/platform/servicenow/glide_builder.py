...
class GlideQueryBuilder(BaseQueryBuilder['GlideQueryBuilder']):
    ...
    def create_condition(self, field: str, operator: Union[BaseOperator, GlideOperator], value: Any) -> GlideCondition:
        ...
        return GlideCondition(field, operator, value)  # type: ignore[arg-type]
    ...
    def contains(self, field: str, value: str) -> 'GlideQueryBuilder':
        ...
        return self.where(field, GlideOperator.CONTAINS, value)  # type: ignore[arg-type]
    
    def starts_with(self, field: str, value: str) -> 'GlideQueryBuilder':
        """Add STARTS WITH condition."""
        return self.where(field, GlideOperator.STARTS_WITH, value)  # type: ignore[arg-type]
    
    def ends_with(self, field: str, value: str) -> 'GlideQueryBuilder':
        """Add ENDS WITH condition."""
        return self.where(field, GlideOperator.ENDS_WITH, value)  # type: ignore[arg-type]
    
    def is_empty(self, field: str) -> 'GlideQueryBuilder':
        """Add IS EMPTY condition."""
        return self.where(field, GlideOperator.IS_EMPTY, None)  # type: ignore[arg-type]
    
    def is_not_empty(self, field: str) -> 'GlideQueryBuilder':
        """Add IS NOT EMPTY condition."""
        return self.where(field, GlideOperator.IS_NOT_EMPTY, None)  # type: ignore[arg-type]
    ...
    def from_natural_language(cls, query: str, table: str) -> 'GlideQueryBuilder':
        ...
        if any(word in query_lower for word in ['urgent', 'critical', 'high priority']):
            builder.where('priority', GlideOperator.IN, ['1', '2'])  # type: ignore[arg-type]
        
        # State detection
        if 'open' in query_lower:
            builder.where('state', GlideOperator.IN, ['1', '2', '3'])  # type: ignore[arg-type]  # New, In Progress, On Hold
        elif 'closed' in query_lower:
            builder.where('state', GlideOperator.EQUALS, '7')  # type: ignore[arg-type]  # Closed
        
        # Assignment detection
        if 'assigned to me' in query_lower:
            builder.where('assigned_to', GlideOperator.EQUALS, 'javascript:gs.getUserID()')  # type: ignore[arg-type]
        elif 'unassigned' in query_lower:
            builder.is_empty('assigned_to')
        
        # Time-based queries
        if 'today' in query_lower:
            builder.where('sys_created_on', GlideOperator.ON, 'Today@javascript:gs.beginningOfToday()@javascript:gs.endOfToday()')  # type: ignore[arg-type]
        elif 'this week' in query_lower:
            builder.where('sys_created_on', GlideOperator.AT_OR_AFTER, 'javascript:gs.beginningOfThisWeek()')  # type: ignore[arg-type]
